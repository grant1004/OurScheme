...
struct MAP {
  ...
  vector<EXP> vec ; // value or function body
  FuncType funcType ; 
};
...
class SyntaxErrorException
{
  ...
  SyntaxErrorException( ExceptionType ErrType, EXP token ) 
  {
    ...
    else if ( ErrType == SYNERR_RIGHTPAREN ) 
    {
      ss << "ERROR (unexpected token) : "
...
void FixSystemPrimitiveAndNil( vector<EXP> &s_exp ) {
  ...
  while ( i < s_exp.size() ) {
    ...
    else if ( s_exp.at( i ).type == LAMBDA ) 
      s_exp.at( i ).token = "#<procedure lambda>" ;
    else if ( s_exp.at( i ).type == LEFT_PAREN && i+1 < s_exp.size() 
              && s_exp.at( i + 1 ).type == RIGHT_PAREN ) {
      s_exp.at( i ).token = "nil" ;
...
void DeleteQuote( vector<EXP> & result )
{
  int parnum = 0 ; 

  if ( result.at( 0 ).type == LEFT_PAREN && result.at( 1 ).type == QUOTE )
  {
    result.pop_back() ; 
...
private: 
  ...
  void PreordeTransValue( EXP* focusNode )
  {
    ...
    if ( focusNode != NULL ) {
      ...
      PreordeTransValue( focusNode->listPtr ) ;
      PreordeTransValue( focusNode->next ) ;
    } // if 
  } // PreordeTransValue()

  vector<MAP> GetParameter( EXP * first ) ; 
  ...
  bool InsertMapAndPar( string str, vector<EXP> vec, vector<string> pp, FuncType type ) { 
    // insert parameter/ sexp/ type to global map
    int i = 0 ;
    while ( i < msymbolMap.size() ) {
      if ( msymbolMap.at( i ).str == str ) {
        msymbolMap.at( i ).vec.clear() ; 
        msymbolMap.at( i ).vec.assign( vec.begin(), vec.end() ) ;
        msymbolMap.at( i ).par_name.clear() ; 
  ...
  bool InsertMap( string str, vector<EXP> vec ) { 
  ...
  bool InsertLocalMap( string str, vector<EXP> vec ) { 
    int i = 0 ;
    while ( i < mlocalsymbolMap.at( mlocalsymbolMap.size()-1 ).parameter.size() ) {
      if ( mlocalsymbolMap.at( mlocalsymbolMap.size()-1 ).parameter.at( i ).str == str ) {
        mlocalsymbolMap.at( 
                            mlocalsymbolMap.size()-1 ).parameter.at( i ).vec.assign( vec.begin(), 
                                                                                     vec.end() ) ;
        return true ;
      } // if
    ...
    mm.str = str ;
    mm.vec.assign( vec.begin(), vec.end() ) ;
    mlocalsymbolMap.at( mlocalsymbolMap.size()-1 ).parameter.push_back( mm ) ;
    return false ;
  ...
  bool IsList( vector<EXP> vec )
  {
    ...
    bool isList = true ; 
    int parNum = 0 ; 
    EXP * root = DynamicBuildTree( vec, i ) ;
    // cout << "Root:" ; PreOrderTraversal( root ) ; 
    // cout << endl ; 
    EXP * now = root->listPtr ; // "(" 
  ...
  bool TryGetGlobalMAPValue( string key, MAP & value )
  {
    
    for ( int i = 0 ; i < msymbolMap.size() ; i ++ )
    {
      if ( msymbolMap.at( i ).str == key )
      {
        value = msymbolMap.at( i ) ; 
        return true ;
      } // if 
  ...
  } // TryGetGlobalMAPValue()

public :
  void ClearLSymbolMap() ;
  ...
  void Inserting( MAP input, vector<MAP> & dst ) 
  {
    // cout << "name :" << input.str << endl ; 
    // cout << "vec : " << PrettyString( input.vec ) << endl ; 
    int i = 0 ;
    bool has = false ; 
    while ( i < dst.size() && has == false ) {
      if ( dst.at( i ).str == input.str ) {

        dst.at( i ).vec.assign( input.vec.begin(), input.vec.end() ) ;
        dst.at( i ).funcType = input.funcType ; 
        dst.at( i ).par_name.assign( input.par_name.begin(), input.par_name.end() ) ; 
        has = true ;
      } // if
    ...
    if ( has == false )
    {
      MAP mm ;
      mm.str = input.str ;
      mm.vec.assign( input.vec.begin(), input.vec.end() ) ;
      mm.funcType = input.funcType ;
      dst.push_back( mm ) ;
    } // if 

  } // Inserting() 

  bool Finding( string key, vector<MAP> vec, MAP & value  )
  {
    value.str = "-1" ; 
    for ( int i = 0 ; i < vec.size() ; i ++ )
    {
...
void Functions::PrintMap() {
  ...
  {
    cout << "Stack level : " << mlocalsymbolMap.size() << endl ; 
    while ( i < mlocalsymbolMap.at( mlocalsymbolMap.size()-1 ).parameter.size() ) {
      cout << mlocalsymbolMap.at( mlocalsymbolMap.size()-1 ).parameter.at( i ).str << ": " ;
      k = 0 ;
      while ( k < mlocalsymbolMap.at( mlocalsymbolMap.size()-1 ).parameter.at( i ).vec.size() ) {
        cout << mlocalsymbolMap.at( mlocalsymbolMap.size()-1 ).parameter.at( i ).vec.at( k ).token ;
        k++ ;
      } // while
...
void Functions::ClearLocalMap() {
  mlocalsymbolMap.at( mlocalsymbolMap.size()-1 ).parameter.clear() ;
} // Functions::ClearLocalMap()

void Functions::ClearLSymbolMap()
{
  mlocalsymbolMap.clear() ; 
} // Functions::ClearLSymbolMap()
...
bool Functions::FindMap( string str, vector<EXP> &new_vector ) { // find local first then find global 
  ...
  if ( FindLocalMap( str, new_vector ) ) 
  {
    if ( new_vector.at( 0 ).type == LEFT_PAREN && new_vector.at( 1 ).type == QUOTE )
    {
      int i = 0 ; 
      EXP* root = DynamicBuildTree( new_vector, i ) ;
      mexeNode = root ; 
      Eval() ; 
      new_vector.clear() ; 
      new_vector.assign( root->vec.begin(), root->vec.end() ) ; 
    } // if 

    return true ; 
  } // if 
  else if ( FindGlobalMap( str, new_vector, ss, type ) ) 
  {
    if ( type == TYPENONE )
    {
      if ( new_vector.at( 0 ).type == LEFT_PAREN && new_vector.at( 1 ).type == QUOTE )
    ...
    // cout << "HERE : " << PrettyString( new_vector ) ;
    return true ; 
  } // else if 
...
bool Functions::FindLocalMap( string str, vector<EXP> &new_vector ) { // only find local map
  ...
  while ( i < mlocalsymbolMap.at( mlocalsymbolMap.size()-1 ).parameter.size() ) { 
    if ( mlocalsymbolMap.at( mlocalsymbolMap.size()-1 ).parameter.at( i ).str == str ) {
      new_vector.assign( mlocalsymbolMap.at( mlocalsymbolMap.size()-1 ).parameter.at( i ).vec.begin(), 
                         mlocalsymbolMap.at( mlocalsymbolMap.size()-1 ).parameter.at( i ).vec.end() ) ;
      return true ;
    } // if
...
void Functions::If()
{
  ...
  {
    // PreOrderTraversal( decide->listPtr ) ; 
    mexeNode = decide ; 
    Eval() ; 
    emptyptr->vec.clear() ; 
...
{ 
  ...
  if ( CheckNumOfArg( 1 ) )
  {
    ...
    {
      // cout << "List : " << PrettyString( mnonListVec ) << endl ; 
...
void Functions::Cons() { 
  ...
  // cout << "Cons :" ; PreOrderTraversal( mexeNode ) ; cout << endl ; 
  ...
  if ( CheckNumOfArg( 2 ) ) {
    ...
    while ( temp != NULL && temp->type != RIGHT_PAREN ) {
      if ( temp->type != SYMBOL && IsATOM( temp ) ) {
        ...
        // cout << "new vec " << PrettyString( new_vector ) ; 
        for ( int i = 0; i < new_vector.size() ; i++ ) {
          emptyptr->vec.push_back( new_vector.at( i ) ) ;
        } // for
...
void Functions::CallFunction() {
  // PreOrderTraversal( gRoot ) ; cout << endl ; 
  ...
  // cout << "s_exp : " << ExtentionFunction( s_exp )  << endl ; 
  if ( type == TYPELAMBDA ) 
  {
    // cout << "Is lambda " << endl ;
    ...
    /*
    cout << endl << "call function : " << funcName << " is lambda "<< endl ;
    PrintMap() ;  
    */
    ...
    EXP * first = emptyptr -> listPtr -> next ; 
    first -> pre_next -> next = root ;
    root -> pre_next = first -> pre_next ; 
    ...
    /*
    cout << "New : " ; PreOrderTraversal( gRoot ) ; cout << endl ;  
    cout << "---------------------------------------------------"  ; 
    cout << "---------------------------------------------------"  ; 
    cout << "---------------------------------------------------"  ;
    cout << endl ;
  ...
  else 
  {
    ...
    { 
      ...
      */
      ...
      // put parameter value into function body 
      for ( int i = 0 ; i < s_exp.size() ; i++ ) { 
        // cout << s_exp.at( i ).token << endl ; 
        if ( s_exp.at( i ).type == LET ) 
        {
          i-- ;
          int count = 0 ;
          bool bbreak = false ; 
          ...
          while ( bbreak == false ) {
            ...
            if ( bbreak == false ) {
              ...
              // cout << "\ncount : " << count << "  : "
              // << s_exp.at( i+1 ).token << "  cnt : " << cnt  << "  " << cntPar << endl ;

              bool isPush = false ;
              if ( count >= 3 )
              {  
                // cout << "count : " << count << "  : "
                // << s_exp.at( i ).token << "  cnt : " << cnt  << "  " << cntPar << endl ;
                cnt ++ ;
                if ( cnt >= 2 && cntPar == 2 ) 
                {
                ...
                else if ( cntPar >= 3 )
                {
                  MAP value ; 
                  // cout << endl << "now Check \'" 
                  // << s_exp.at( i + 1 ).token << "\' is in CheckMap or not " << endl ; 
                  if ( Finding( s_exp.at( i + 1 ).token, checkMap, value  ) == false )
                  {
              ...
              {
                // cout << "count : " << count << "  : "<< s_exp.at( i ).token 
                // << "  cnt : " << cnt  << "  " << cntPar << endl ;
                if ( cntPar >= 3 )
                {
                  MAP value ; 
                  // cout << endl << "now Check \'" << s_exp.at( i + 1 ).token 
        ...
        else if ( s_exp.at( i ).type == LAMBDA )
        {
          ...
          while ( parNum != 0 )
          {
            ...
            new_s_exp.push_back( s_exp.at( i ) ) ;
            // cout << "push : " << new_s_exp.back().token << endl ;
            i ++ ; 
...
{ 
  EXP ex ;
  InitExp( ex ) ;
  // cout << endl << "CallLambdaFunction() " << mexeNode->token << endl ; 

  EXP* emptyptr = mexeNode->pre_next->pre_listPtr ;
  // cout << "lambda exp : " ; PreOrderTraversal( emptyptr->listPtr ) ; cout << endl ; 

  // cout << emptyptr -> token << endl ; 
  ...
  if ( CheckNumOfArg( parameterSize ) )
  {
    ...
    rr.token = ")" ; 
    rr.type =  RIGHT_PAREN ; 
    new_s_exp.push_back( rr ) ; 

    rr.token = "begin" ; 
    rr.type =  BEGIN ; 
    new_s_exp.insert( new_s_exp.begin(), rr ) ;

    rr.token = "(" ; 
    rr.type =  LEFT_PAREN ; 
    new_s_exp.insert( new_s_exp.begin(), rr ) ;

    // cout << "Extention Func : " << endl << PrettyString( new_s_exp ) << endl ; 

    EXP* root = DynamicBuildTree( new_s_exp, i ) ;
    // cout << "Ori " ; PreOrderTraversal( gRoot ) ; cout << endl ; 
    // cout << "empty exp " ; PreOrderTraversal( emptyptr ) ; cout << endl ;  
...
string GetFuncType( FuncType type ) 
{
  ...
  else if ( type == TYPEDEFINE )
  {
    str = "Function" ;
  } // else if 
  else if ( type == TYPELAMBDA )
  {
    str = "Lambda Function" ;
  } // else if 
...
void Functions::Define() { 
  ...
  if ( CheckNumOfArg( 0 ) || CheckNumOfArg( 1 ) ) { 
    ...
    if ( temp->type == SYMBOL ) { // project1 + 2
      ...
      if ( temp->type == EMPTYPTR ) 
      { 
        if ( temp->listPtr->next->token == "lambda" )
        {
          type = TYPELAMBDA ; 
          MAP mm ; 
          mm.funcType = TYPELAMBDA ; 
          mm.str = str ; 
          vector<EXP> lambda_exp ; 
          GetPreOrderTraversalWithNoEmpty( temp->listPtr, lambda_exp ) ; 
          mm.vec.assign( lambda_exp.begin(), lambda_exp.end() ) ; 
          mm.vec.at( 0 ).memSpace = memNum ; 
          // cout << "Define Lambda : " << mm.str << endl ; 
          Inserting( mm, msymbolMap ) ; 
        } // if
        else if ( temp->listPtr->next->type == QUOTE )
        {
          type = TYPENONE ; 
          MAP mm ; 
          mm.funcType = type ; 
          mm.str = str ; 
          vector<EXP> exp ; 
          GetPreOrderTraversalWithNoEmpty( temp->listPtr, exp ) ; 
          mm.vec.assign( exp.begin(), exp.end() ) ; 
          // cout << "Define Lambda : " << mm.str << endl ; 
          mm.vec.at( 0 ).memSpace = memNum ; 
          Inserting( mm, msymbolMap ) ;
        } // else if 
        else 
        {
          ...
          InsertMapAndPar( str, temp->vec, pp, TYPENONE ) ;
        ...
        if ( type == TYPEDEFINE ) {
          ...
          InsertMapAndPar( str, new_vector, pp, TYPENONE ) ;
        ...
        vs.push_back( ex ) ; 
        pp.clear() ; 
        InsertMapAndPar( str, vs, pp, type ) ;
        // InsertMap( str, vs ) ;
  ...
  // cout << "Defined " << str << " and this is " << GetFuncType( type ) << endl ;  
...
void Functions::Eval() {
  ...
  // cout << "mexeNode in Eval is : " << mexeNode->token << endl ;
  ...
  if ( IsATOM( temp ) && temp->type != SYMBOL  )  {
    ...
    else
    {
      ...
      if ( tt == TYPENONE )
      {
        ...
        int i = 0 ; 
        EXP * root = DynamicBuildTree( new_vector, i ) ; 
        if ( root->type == EMPTYPTR && root->listPtr->next->type == QUOTE )
        {
          mexeNode = root ; 
          Eval() ; 
          mresult.assign( root->vec.begin(), root->vec.end() ) ;
        } // if 
        else 
        {
          mresult.assign( new_vector.begin(), new_vector.end() ) ;
      ...
      else if ( tt == TYPELAMBDA )
      {
        mresult.clear() ; 
        EXP t ; 
        t.token = "#<procedure lambda>" ;
        t.type = SYMBOL ; 
        mresult.push_back( t ) ;
      } // else if 
  ...
  { // temp == emptyptr || temp == bounding symbol 
    ...
    if ( FindMap( firstArgument->token, new_vector ) )
    {
      ...
      else if ( IsSystemPrimitive( new_vector.at( 0 ).type ) )
      {
        ...
        {
          // cout << "Now is " << firstArgument->token << endl ; 
          firstArgument->token = new_vector.at( 0 ).token ; 
          firstArgument->type = new_vector.at( 0 ).type ; 
        } // if 
    ...
    { 
      // cout << "HERE " << firstArgument->token << endl ; 
      ...
      { 
        ...
        { 
          ...
          {
            // cout << ">>>>now is :" << firstArgument->type<< " is define func " << endl ;
            if ( mlevel > 1 )
            { 
              throw new ErrorLevelException( firstArgument->token ) ; 
            } // if
          ...
          { 
            // cout << ">>>>now is :" << firstArgument->type<< " is cond func " << endl ;
          } // else if 
          else if ( firstArgument->type == LET )
          {
            // cout << ">>>>now is :" << firstArgument->type<< " is let func " << endl ;
            // CheckLetFormat() ; 
          } // else if 
          else if ( firstArgument->type == LAMBDA )
          { 
            // cout << ">>>>now is :" << firstArgument->type<< " is lambda exp" << endl ;
          } // else if 
          else if ( firstArgument->type == LAMBDA_FUNC )
          { 
            // cout << "now is :" << mexeNode->token << " is lambda func " << endl ;
          } // else if
  ...
  // cout << "firstArgument in Eval is : " << firstArgument->token << endl ;
