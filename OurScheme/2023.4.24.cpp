...
private: 
  ...
  bool InsertMapAndPar( string str, vector<EXP> vec, vector<string> pp, FuncType type ) { 
    // insert parameter/ sexp/ type 到 global map
    int i = 0 ;
  ...
  bool InsertMap( string str, vector<EXP> vec ) { // return true已經在裡面更改值 
  ...
  bool InsertLocalMap( string str, vector<EXP> vec ) { // local variable用 
    int i = 0 ;
...
bool Functions::FindMap( string str, vector<EXP> &new_vector ) { // 先 find local -> global 
  int i = 0 ; 
...
bool Functions::FindGlobalMap( string str, vector<EXP> &new_vector, vector<string> &ss, FuncType &type ) { // 只find global map
  int i = 0 ;
  while ( i < msymbolMap.size() ) {
    if ( msymbolMap.at( i ).str == str ) {
      new_vector.assign( msymbolMap.at( i ).vec.begin(), msymbolMap.at( i ).vec.end() ) ;
...
bool Functions::FindLocalMap( string str, vector<EXP> &new_vector ) { // 只find local map
  int i = 0 ; 
  while ( i < mlocalsymbolMap.size() ) { // find local map
    if ( mlocalsymbolMap.at( i ).str == str ) {
      new_vector.assign( mlocalsymbolMap.at( i ).vec.begin(), mlocalsymbolMap.at( i ).vec.end() ) ;
          ...
          if ( temp->type == EMPTYPTR ) {
            ...
            while ( expression != temp->next ) {
              ...
              if ( expression->type == RIGHT_PAREN ) {
                ...
                while ( expression->type != LEFT_PAREN ) { // 走回去 
